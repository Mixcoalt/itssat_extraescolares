...
<?php
	...
	$resultado=mysqli_query($conex,$sql);
	while($fila=mysqli_fetch_assoc($resultado)){
		$imagen='../../Imagen/'.$fila['nombre_imagen'];
?>
<img src="<?php echo ($imagen);?>" alt="Imagen" title="Logos" align="center"/>
<?php
}
?>
...
?>
	<li class="menu__item"><a href="../../Php/cerrarSesion.php" class="menu__link">Cerrar Sesión</a></li>
      ...
      <form action="php/cargarLogo.php" enctype="multipart/form-data" method="post" >
	  </table>
			<h2>Cargar logotipos</h2>
			<table>
			<thead>
			<tr>
				<th>Descripcion </th>
			...
			<tr>
			<td><select id="tipoIcon" name="tipoIcon" >
				<?php
					$sql1="SELECT * FROM logos"; //Consulta
				...
				?>
					 <option value="<?php echo $mostrar1['id_logos']?>"><?php echo $mostrar1['descripcion_logos']?></option>
				<?php } mysqli_free_result($resultado1); //Cierre del While ?> 
			</select></td>
			<td><input type="file" name="imagen" id="imagen" required="obligatorio"></td>
			<td><button class="boton" type="submit" name="enviar" id="enviar" onclick="return mensajes()">cargar</button></td>
		</tr>
